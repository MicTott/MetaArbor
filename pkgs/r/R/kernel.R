# TreeNeighbor measurement kernel (DESIGN.md §3a).
#
...
tn_lognorm <- function(counts, assume_log = FALSE, lib = NULL) {
...
tn_variable_genes <- function(expr_a, expr_b, n_top = 1000) {
...
tn_rank_normalize <- function(expr) {
...
#'
#' @param test_norm,train_norm outputs of tn_rank_normalize on the joint HVGs
...
tn_vote_cache <- function(test_norm, train_norm, train_labels, chunk = 2000L) {
...
tn_aggregate_cache <- function(cache, mapping) {
...
tn_auroc <- function(scores, positive) {
  n_pos <- sum(positive)
  n_neg <- sum(!positive)
...
tn_node_auroc <- function(cache, test_labels, query, node_leaves, negatives = NULL) {
...
tn_leaf_costs <- function(cache_a, labels_a, cache_b, labels_b) {
...
tn_measure <- function(expr_a, labels_a, expr_b, labels_b,
                       n_hvg = 1000, assume_log = FALSE, chunk = 2000L,
                       lib_a = NULL, lib_b = NULL) {
  ea <- tn_lognorm(expr_a, assume_log, lib_a)
  eb <- tn_lognorm(expr_b, assume_log, lib_b)
  hvg <- tn_variable_genes(ea, eb, n_hvg)
  na <- tn_rank_normalize(ea[hvg, , drop = FALSE])
  nb <- tn_rank_normalize(eb[hvg, , drop = FALSE])
  cache_a <- tn_vote_cache(na, nb, labels_b, chunk)  # A cells vs B leaves
  cache_b <- tn_vote_cache(nb, na, labels_a, chunk)  # B cells vs A leaves
  costs <- tn_leaf_costs(cache_a, labels_a, cache_b, labels_b)
