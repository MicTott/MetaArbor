# The frozen TreeNeighbor walk estimator (repo NOTES.md items 4, 7-9;
...
tn_boot_delta <- function(scores1, scores2, positive, rng, n_boot = 200L) {
  ...
  for (b in seq_len(n_boot)) {
    sp <- ip[tn_minstd_indices(rng, np, np) + 1]
    sn <- ineg[tn_minstd_indices(rng, nn, nn) + 1]
    idx <- c(sp, sn)
    delta[b] <- tn_auroc(scores1[idx], posmask) - tn_auroc(scores2[idx], posmask)
  }
  delta
}
...
tn_select_node <- function(cache, test_labels, query, tree, seed,
  ...
  current <- "root"
  repeat {
    kids <- sort(tn_children(tree, current))
...
tn_baseline_map <- function(cache, test_labels, tree, S_dir, base_seed = 7,
                            alpha = 0.05, n_boot = 200L, min_auroc = 0.6,
  ...
  res <- lapply(seq_along(queries), function(i) {
    q <- queries[i]
    sel <- tn_select_node(cache, test_labels, q, tree, seed = base_seed + i - 1,
